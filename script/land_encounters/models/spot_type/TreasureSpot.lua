...
function TreasureSpot:trigger_event(context)
    local human_character = context:family_member():character()

    if self:is_human_and_it_is_its_turn(human_character) then 
        local faction_cqi = human_character:faction():command_queue_index()
        local incident_key = self.event
        local target_faction_cqi = 0 -- ignore flag
        local secondary_faction_cqi = 0
        local character_cqi = human_character:command_queue_index()
        local military_force_cqi = human_character:military_force():command_queue_index()
        local region_cqi = 0
