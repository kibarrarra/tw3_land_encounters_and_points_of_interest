...
function LandEncounterManager:generate_land_encounters(coordinates_by_zone, turn_number)
...
function LandEncounterManager:restore_from_previous_state(coordinates_by_zone, turn_number)
    -- we create new land encounters given the map coordinates from whatever map was selected
    self:initialize_spots_by_zone(coordinates_by_zone, turn_number)
    -- we restore the data inside each encounter
    self:reinstate_land_encounters(self.previous_state, turn_number)
end
...
function LandEncounterManager:reinstate_land_encounters(previous_state, turn_number)
    for i = 1, #self.zones do
        ...
        if active_battle_spot_index ~= nil then
            ...
            self.invasion_battle_manager:reset_state_post_battle(self.zones[i], active_battle_spot_index, self.invasion_battle_manager.DEFAULT_ENEMY_INVASION)
        end
    end
end
...
function LandEncounterManager:newFrom(core, invasion_battle_manager, battle_event_factory)
    local t = { zones = {}, active_spot_percentage = DEFAULT_ACTIVE_SPOT_PERCENTAGE, battle_percentage = DEFAULT_BATTLE_PERCENTAGE, core = core, invasion_battle_manager = invasion_battle_manager, battle_event_factory = battle_event_factory }
    setmetatable(t, self)
    self.__index = self
    return t
end
